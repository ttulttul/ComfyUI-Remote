...
- Enable `Force Modal Rebuild` to embed a unique build nonce so Modal rebuilds the remote image even when it would otherwise reuse cached layers.
- Enable `Delete Remote App` to call `modal app delete` before deployment, wiping the remote image/app so Modal definitely rebuilds from scratch.
