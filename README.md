...
- A `utils` shim module is written next to the generated workflow so any `import utils.*` statements are forced to load `/workspace/ComfyUI/utils` even if another package named `utils` is present.
