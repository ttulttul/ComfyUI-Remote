...
- Added a `Force Modal Rebuild` option that injects a build nonce into the generated Modal project so the remote image rebuilds even when Modal's cache would normally skip it.
